...
export async function fetchMarketData(
  ...
  if (liveStockPrice === null && typeof window !== 'undefined') {
    try {
      ...
      const snapshotUrl = `${cleanBase}market-data.json`;
      const snapshotResp = await fetch(snapshotUrl, { signal: AbortSignal.timeout(3500) });
      if (snapshotResp.ok) {
      ...
      // Continue to external fallbacks
    }
  }
  ...
  const finalResult: MarketDataResult = {
    ...
    isLiveStock,
    isLiveFx,
    errorMessage: !isLiveStock && !isPrevClose && !localCache
